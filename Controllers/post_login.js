...
module.exports = (req, res) => {
  ...
  db.query(
    `SELECT users.id,users.first_name,users.email,users.password,users.business_id,categories.id AS category_id
			  FROM users
    ...
    (err, result) => {
      ...
      if (result.rows.length === 0) {
        ...
        bcrypt.compare(
          ...
          (err, match) => {
            console.log("RESSSSSSSSSSSSSSSSSSSSSSSSSSSUKT+", result.rows[0]);
            user = {
