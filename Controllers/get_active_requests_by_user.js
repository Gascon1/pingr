...
module.exports = (req, res) => {
	...
	let query = `SELECT requests.id AS request_id, businesses.name AS business_name, businesses.address AS business_address, categories.name AS category, services.name AS service, services.price AS price, services.duration AS duration, statuses.name AS status, requests.appointment_start_time, requests.appointment_end_time, requests.start_time AS availability_start_time, requests.end_time AS availability_end_time FROM requests 
