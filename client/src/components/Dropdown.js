...
export default function Dropdown(props) {
  ...
  const categoriesList = state.categories.map(category => { return <option data-id={category.id.toString()}>{category.name}</option> })

  const servicesList = props.services.map(service => { return <option>{service.name}</option> })
  ...
  return (
    ...
    >
      {props.list === "categoryList" && <option disabled selected>Choose category</option>}
      {props.list === "categoryList" && categoriesList}

      {props.list === "serviceList" && <option disabled selected>Choose service</option>}
